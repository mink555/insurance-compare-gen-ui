...
> **[insurance-compare](https://github.com/mink555/insurance-compare)** 프로젝트의 Generative UI 데모입니다.  
> 기존 프로젝트가 PDF 파싱 → 데이터 구조화 → 분석 리포트를 생성했다면,  
> 이 데모는 같은 분석 결과를 **AI가 런타임에 UI를 결정해서 보여주면 어떨까?** 를 탐색합니다.
...
`insurance-compare`는 라이나생명 vs 한화생명 암보험 특약을 비교하는 프로젝트입니다.  
PDF 상품요약서를 파싱해 구조화된 데이터로 만들고, 인사이트 리포트를 생성합니다.

이 데모는 그 분석 결과 데이터를 가져와서 다른 질문을 던져봤습니다:
...
기존 방식은 분석 결과를 **정해진 UI**에 채워넣습니다.  
Generative UI는 LLM이 질문을 분석해서 **어떤 UI를 그릴지** 런타임에 결정합니다.
...
같은 데이터, 같은 화면인데 **질문에 따라 완전히 다른 UI**가 나옵니다.
...
질문하면 LLM이 3개의 툴 중 어떤 컴포넌트를 렌더링할지 선택합니다.
...
질문하면 LLM이 **대시보드 레이아웃 전체**를 JSON으로 설계합니다.  
위젯 종류, 개수, 배치 순서를 모두 LLM이 결정합니다.
